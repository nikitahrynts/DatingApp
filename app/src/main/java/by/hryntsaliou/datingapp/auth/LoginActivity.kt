...
import by.hryntsaliou.datingapp.databinding.ActivityLoginBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.verifyOtp.setOnClickListener {
            if (binding.userOtp.text!!.isEmpty()) {
                binding.userOtp.error = "Please, enter your verification code!"
            } else {
...
//                binding.sendOtp.showNormalButton()
        ...
        val options = PhoneAuthOptions.newBuilder(auth)
            .setPhoneNumber("+375$number")       // Phone number to verify
...
//            binding.sendOtp.showNormalButton()
    ...
    private fun checkUserExist(number: String) {
        ...
        FirebaseDatabase.getInstance().getReference("users").child("+375$number")
            .addValueEventListener(object : ValueEventListener {
                override fun onCancelled(p0: DatabaseError) {
                    dialog.dismiss()
                    Toast.makeText(
                        this@LoginActivity, p0.message, Toast.LENGTH_SHORT
                    ).show()
                }
